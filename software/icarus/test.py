...
def test_portfolio():
    ...
    for ticker, retriever in retrievers.items():
        ...
        div_yield = retriever.get_dividend_yield() # Already decimal, e.g., 0.0111
        
        div_yield = div_yield if div_yield is not None else Decimal('0')
        ...
        # Assume 100 shares of each
        total_value += price * 100

    # Calculate average dividend yield (as decimal)
    avg_div_yield = Decimal('0')
    if portfolio_data:
        avg_div_yield = sum(d['div_yield'] for d in portfolio_data.values()) / Decimal(str(len(portfolio_data)))
    
    # Project growth for 10 years (use decimal yield directly)
    results = project_portfolio_growth(
        tax_calculator=FinnishCorporateTaxCalculator(),
        initial_value=total_value,
        dividend_yield=avg_div_yield,  # Already decimal, e.g., 0.0111
    ...
    # Print portfolio details (convert to percentage for display)
    print("\nPortfolio Details:")
